...
export const SpectatorScreen = () => {
  const {
    players, phase, setPhase, round, addMessage, messages,
    activeEmotes, triggerEmote, usdcBets, usdcBalance,
    placeBetUSDC, resetGame,
  } = useGameStore();
  ...
  // Simulated live odds
  const [odds, setOdds] = useState({ mafia: 3.2, citizens: 1.45 });

  // Market feed
  const [marketFeed, setMarketFeed] = useState<string[]>([]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (Math.random() > 0.5) {
        const names = ['Whale_0x', 'DeFiKing', 'Anon42', 'AlphaBet', 'MonadMax', 'X402Pro'];
        const amounts = [1.0, 2.5, 5.0, 10.0, 25.0];
        const targets = ['Mafia', 'Citizens'];
        const name = names[Math.floor(Math.random() * names.length)];
        const amt = amounts[Math.floor(Math.random() * amounts.length)];
        const target = targets[Math.floor(Math.random() * targets.length)];
        setMarketFeed(prev => [`${name} bet $${amt} USDC on ${target}`, ...prev].slice(0, 5));
      }
      // Fluctuate odds
      setOdds(prev => ({
        mafia: Math.max(1.1, prev.mafia + (Math.random() - 0.5) * 0.15),
        citizens: Math.max(1.1, prev.citizens + (Math.random() - 0.5) * 0.1),
      }));
    }, 3000);
  ...
  useEffect(() => {
    ...
    const specLines = [
      ...
      'lol Scarlet is playing it cool',
      'anyone else think Cipher is mafia?',
  ...
  useEffect(() => {
    ...
    const runPhase = () => {
      if (phase === GamePhase.DAY_DISCUSSION) {
        ...
        const interval = setInterval(() => {
          if (Math.random() > 0.5) {
            const aliveAI = players.filter(p => !p.isDead);
  ...
  }, [phase, players, round, addMessage, setPhase, triggerEmote]);
            ...
            >
              ...
              <div className="h-12 bg-purple-500/10 border-b border-purple-500/20 flex items-center justify-between px-4 shrink-0">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[9px] text-white/30 font-mono">{Math.floor(12 + Math.random() * 30)} online</span>
                </div>
              </div>
        ...
        <div className="hidden md:flex w-[380px] shrink-0 h-full border-l border-white/5 z-50 flex-col bg-[#030712]/95 backdrop-blur-xl">
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="p-4 border-b border-white/5">
              ...
              <div className="grid grid-cols-2 gap-2">
                <GlassCard className="p-3 text-center bg-red-500/5 border-red-500/20">
                  ...
                  <div className="text-2xl font-mono font-black text-red-400">{odds.mafia.toFixed(2)}x</div>
                </GlassCard>
                <GlassCard className="p-3 text-center bg-green-500/5 border-green-500/20">
                  ...
                  <div className="text-2xl font-mono font-black text-green-400">{odds.citizens.toFixed(2)}x</div>
                </GlassCard>
              </div>
            </div>
                  ...
                  {showTargetDropdown && (
                    ...
                    >
                      {getTargetOptions().map(opt => (
                        ...
                        >
                          {/* Show avatar if it's a player name */}
              ...
              {betAmount && betTarget && (
                ...
                >
                  ...
                  <span className="text-sm font-mono font-bold text-green-400">
                    ${(parseFloat(betAmount || '0') * (betTarget === 'Mafia' ? odds.mafia : odds.citizens)).toFixed(2)} USDC
                  </span>
                </motion.div>
              )}
            ...
            <div className="p-4">
              ...
              <div className="space-y-1.5">
                {marketFeed.length === 0 ? (
                  <div className="text-center py-3 text-white/20 text-xs">Awaiting transactions...</div>
                ) : (
                  marketFeed.map((feed, i) => (
                    <motion.div
                      key={`${feed}-${i}`}
                      initial={{ opacity: 0, y: -5 }}
                      animate={{ opacity: 1, y: 0 }}
                      className="text-[11px] font-mono text-white/50 flex items-center gap-2"
                    >
